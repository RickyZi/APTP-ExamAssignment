...
(:init
    ...
    (= (num_crates_carrier carr0) 0) ; at the beginning carr0 has 0 crates (closed world assumption)
)

(:goal (and
    
    ;Alice wants only food
    (or (has_crate alice f0) (has_crate alice f1) (has_crate alice f2) (has_crate alice f3))

    ; bob wants only medicine
    (or (has_crate bob m0) (has_crate bob m1) (has_crate bob m2) (has_crate bob m3))

    ; mary wants food and medicine
    (or (has_crate mary f0) (has_crate mary f1) (has_crate mary f2) (has_crate mary f3))
    (or (has_crate mary m0) (has_crate mary m1) (has_crate mary m2) (has_crate mary m3))

    ; dave doesn't want neither food or medicine
    (not (or (has_crate dave f0) (has_crate dave f1) (has_crate dave f2) (has_crate dave f3)))
    (not (or (has_crate dave m0) (has_crate dave m1) (has_crate dave m2) (has_crate dave m3)))

    ; -----------------------------------------------
    ; additional goal: make robot (and carrier) go back to depot for new delivery
    (robot_at agent depot)
    (carrier_at carr0 depot)
