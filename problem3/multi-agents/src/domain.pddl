...
; domain file for problem 3.3 (extension of problem 3.2) -> multi-agents version
...
(:predicates 
    ...
    ; check how has the crates, carrier or person
    ...
    (has_carrier ?r - robot ?k - carrier) ; enforce to which robot each carrier belongs 
    ...
    (crate_content ?c - crate ?rc - supply) ; def what's the content of the crate
    (needs ?p - person ?rc - supply) ; person need a certain crate c (it may contain medicine, food, ....)
    (does_not_need ?p - person ?rc - supply) ; person p doesn't need a certain crate c (containing food or medicine or ...)

)

;note: a robotic agent cannot pick up several crates at the same time
; same hp as in pb 3.2: agent can pick up one crate at a time
(:durative-action load
    ...
    :condition (and 
        (at start (and 
            ...
            (num_crates ?k ?startq) ; check carrier k is actually holding startq crates, and keep this condition true throughout the exect of the action (over all)
...
(:durative-action unload
    :parameters (?r - robot ?c - crate ?rc - supply ?p - person ?k - carrier ?l - location ?startq ?endq - quantity)
    :duration (= ?duration 4) ; slightly more time due to interaction with person
    ...
    :condition (and 
        (at start (and 
            ...
            (num_crates ?k ?startq)
            (crate_content ?c ?rc)
            (needs ?p ?rc) ; give to person p the desired resource (food/medicine)
