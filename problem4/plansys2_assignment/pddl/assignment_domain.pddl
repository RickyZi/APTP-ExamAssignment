...
; domain file for problem 3.4 (extension of problem 3.3 single agents)

; need to remove constants since are not correctly parsed by plansys2
...
(:types 
    ...
    ; new def of quantity to model the num of crates carried by the carrier
    zero_quantity - crates_quantity
    non_zero_quantity - crates_quantity

)

; REMOVE CONSTANTS SINCE NOT PARSED BY PLANSYS2
; (:constants 
;     agent - robot
...
(:predicates 
    ...
    (inc ?n ?nn - crates_quantity) ; n+1 = nn
    (dec ?n ?nn - crates_quantity) ; n-1 = nn
...
(:durative-action load
    :parameters (?r - robot ?c - crate ?k - carrier ?wh - warehouse ?startq - crates_quantity ?endq - non_zero_quantity)
    :duration (= ?duration 2)
...
(:durative-action unload
    :parameters (?r - robot ?c - crate ?s - supply ?p - person ?k - carrier ?l - location ?startq - non_zero_quantity ?endq - crates_quantity)
...
(:durative-action move_to_depot
    :parameters (?r - robot ?k - carrier ?src - place ?dst - warehouse ?zq - zero_quantity) 
    :duration (= ?duration 5)
    :condition (and 
        (at start (and 
            (robot_at ?r ?src)
        ...
        (over all (and 
            (num_crates ?k ?zq) ; check no other crates will be loaded/unloaded from carrier during exec of the action
        ))
    )
    :effect (and 
        (at start (and 
